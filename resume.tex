...
Michael is a Cyber Security professional with a passion for Blue Team operations and continuous learning. He has extensive experience in managing and remediating diverse cyber incidents, ranging from email account compromises to ransomware. Michael has a strong IT background with expertise in Linux and Windows administration.
\section*{Skills}
\small
\begin{itemize}
    \begin{multicols}{2}
    ...
    \item Cloud Incident Response (AWS, GCP, Azure)
    \item Security Automation (Tines, CrowdStrike Falcon Fusion)
    \end{multicols}
\end{itemize}

\section*{Work History}
\begin{itemize}
    \item\textbf{Senior DevSecOps Engineer, 05/2026 - Present} \
    Cloudera - Remote - Omaha, NE
    \begin{itemize}
        \small
        \leftskip=4em
        \item Engineered and operationalized self-service security platforms that embedded secure-by-default controls into product development workflows and reduced security toil across engineering teams.
        \item Integrated and matured automated security testing, including SAST, DAST, SCA, IAST, secrets detection, and policy enforcement, across CI/CD pipelines to identify and remediate risk earlier in the software lifecycle.
        \item Designed and maintained secure, scalable infrastructure across AWS, Azure, GCP, and on-premises environments using Terraform, Ansible, and Infrastructure as Code best practices.
        \item Deployed, secured, and supported Kubernetes-based security services using Helm, automated policy enforcement, secrets management, and security-as-code practices.
        \item Led security architecture reviews, threat modeling, and technical risk assessments for complex applications, distributed systems, and cloud infrastructure.
        \item Partnered with product engineering, SRE, security operations, incident response, and compliance teams to improve security visibility, production monitoring, and organization-wide adoption of DevSecOps standards.
    \end{itemize}
\begin{itemize}
    \item\textbf{Senior Security Response Engineer, 07/2024 - 05/2026} \\
    Cloudera - Remote - Omaha, NE
    \begin{itemize}
        \small
        \leftskip=4em
        ...
        \item Developed new detections and alerting
...
For 5+ years, I have designed, built, and maintained a production-style homelab environment focused on infrastructure-as-code, container orchestration, and self-hosted services. The platform emphasizes automation, observability, segmentation, and reduced dependency on third-party cloud SaaS platforms. The environment is built using a layered architecture model:

    \begin{itemize}
        \small
        \begin{multicols}{2}
        \item \textbf{Container Orchestration:} Kubernetes (K3s)
        \item \textbf{Container Runtime:} Docker, Podman
        \item \textbf{Infrastructure as Code:} Terraform, Ansible
        \item \textbf{CI/CD \& GitOps:} GitLab CE
        \item \textbf{Virtualization:} Proxmox VE
        \item \textbf{Networking:} Cisco NX-OS/IOS/ASA, TP-Link Omada SDN (VLAN/ACL design)
        \item \textbf{DNS \& PKI Automation:} BIND9, Pi-hole, Cloudflare API, Caddy, NGINX
        \item \textbf{Monitoring \& Observability:} Prometheus, Grafana, Zabbix
        \item \textbf{Systems:} Ubuntu, RHEL 7+, OpenSUSE, FreeBSD
        \item \textbf{Security Tooling:} Security Onion (Snort, Zeek, ELK)
        \item \textbf{Source of Truth:} NetBox (IPAM, virtualization inventory)
        \end{multicols}
    \end{itemize}

This environment includes automated VM/LXC provisioning, dynamic DNS management via Terraform pipelines, Kubernetes cluster deployment across multiple hypervisors, GitLab-based CI/CD for infrastructure and application workloads, and full-stack observability integration.
\end{itemize}
